...
for model in models:
    ...
    y_pred = df['Predicted logFM']
    y_true = df['Observed logFM']
...
df_output.to_csv("./output/rmse_q2.csv", float_format='%.5f', index=False)
...
df_output.to_csv("./output/rmse_q2_diff.csv", float_format='%.5f', index=False)
